...
describe("PostgreSQL schema lifecycle", () => {
    ...
    it("executes schema DDL only through explicit initialization", async () => {
        ...
        expect(tableNames).toHaveLength(62);
        expect(tableNames).toContain("vozeb_pro_tenants");
        expect(tableNames).toContain("vozeb_pro_tenant_memberships");
        expect(ddl).toContain("conrelid = 'vozeb_pro_audit_logs'::regclass");
